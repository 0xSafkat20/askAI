...
export async function generateAnswer(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (
        ...
        reason =
          'Gemini model is not available for this API key or project. Check the configured Gemini model.';
      } else if (response.status === 429) {
        reason = 'Gemini has reached its request or quota limit.';
      } else {
        reason = 'Gemini is temporarily unavailable or busy.';
      }
      return unavailable(
        `${reason} No AI summary was created. Please retry shortly or check the Gemini API key, model, and quota.`,
      );
    }
  ...
  return unavailable(
    `${reason} No AI summary was created. Please retry shortly or check the Gemini API key and quota.`,
  );
}
